      ...
      <div className="mt-14 grid auto-rows-fr gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((svc, i) => (
          <Reveal key={svc.title} delay={(i % 3) * 100} className="h-full">
            <Link
              to={svc.to || '/contact'}
              className="group flex h-full flex-col overflow-hidden rounded-2xl border border-line/10 bg-card shadow-sm transition hover:-translate-y-1 hover:border-accent/40 hover:shadow-xl hover:shadow-navy/10"
            >
              <div className="h-48 shrink-0 overflow-hidden bg-mist">
                <img
                  src={svc.image}
                  alt={svc.title}
                  loading="lazy"
              ...
              <div className="flex flex-1 flex-col p-6">
                ...
                <span className="mt-auto pt-4 inline-block text-sm font-medium text-accent">
                  Learn more →
                </span>
              </div>
            </Link>
